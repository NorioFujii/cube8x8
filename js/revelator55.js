...
function WhiteX(m=0,color=White,ck=false) {  // 白クロスを形成する
    ...
    for (n=n;n<4;) {
        ...
        if (l>0) { 
            ...
            if (f==0) { // 監視範囲になかったので、次へ
                if (Us.slice(-2)==",U") { ui(),Us = Us.slice(0,-2); }
                turnSftND("Y"),Us += ",Y";
                n++;
            }
            else if (f>cube.length) n = 4;
...
function otuge_corner(n=0) { // 下段４コーナーをコーナーお告げに基づき完成する
    ...
    if (n<8) { // 上層コーナー（白上向き）を最下層に降ろす
        ...
        if (Nc(9)==White) {
            ...
            if (l>=0) Us += ",*ねじりセクシーでスロットイン,*#0921282431,f,U,F,R,U2,r";
            else {
...
let yon=false;  // 4x4スイッチ
function fdC(){
    if (yon) s[1]=a[38],a[38]=a[54],a[54]=a[70],a[70]=a[22],a[22]=s[1];
    else     s[1]=a[23],a[23]=a[32],a[32]=a[41],a[41]=a[14],a[14]=s[1];
}
function fdc(){
    if (yon) s[1]=a[22],a[22]=a[70],a[70]=a[54],a[54]=a[38],a[38]=s[1];
    else     s[1]=a[14],a[14]=a[41],a[41]=a[32],a[32]=a[23],a[23]=s[1];
}
...
function getFish(n=0) { // 上面に魚型のクロスを作る
    ...
    if (corCnt==0){ // クロス(10と30が黄色)で、肩からパーム、
        ...
        if (l>=0) {
            ...
            if ((N&1==0)&&(Nc(11)==Nc(14))&&(Nc(20)!=Nc(23))) Us += ",U,x,"+PA+",X"; // パリティ対応
            if ((N&1==0)&&(Nc(11)!=Nc(14))&&(Nc(20)==Nc(23))) Us += ",x,"+PA+",X"; // パリティ対応
...
// 側面に黄ばみのない面の対面で、その面の黄色を避ける２個のキューブを調べる
    ...
    var l = 4, i = -1;
    if (n%2==0) { // １回目（表）
        while ((l-->0)&&!((Nc(37)!=Yellow)&&(Nc(39)!=Yellow))) { uu(); Us += ",U"; }
        ...
        if (i>0) while (++l<4) ui(); // i==0の時、U回転の計数を継続する
    }
    if ((n%2==1)||(i==0)) {  // ２回目（裏）か　上面全黄色
        if (i==0) { l-= 2; n++; uu(); uu(); Us += ",U,U"; }        
        if (Nc(39)==Yellow) { // i,j:0-3:H,L,O,R
            ...
            Us = "*最終交換パーム"; l = 4; i = 0;
            while ((l-->0)&&!((Nc(39)==Nc(38))&&(Nc(38)==Nc(37)))) { uu();  Us += ",U"; }
...
const PA = "*パリティ補正,ml2,U2,F2,ml2,F2,U2,ml2";
const PP = "l2,U2,F2,l2,F2,U2,l2".split(",");
const NP = "r2 B2 U2 l U2 r' U2 r U2 F2 r F2 l' B2 r2".split(" ");
...
function parityAlt(n) {  // 4x4の時自動的に実行される
    ...
    l=4;while ((l-->0)&&!((Nc(2)==Yellow)&&(Nc(8)!=Yellow))) { uu(); rot.push("U"); }
    if (l>=0) // 上層エッジの非黄色が手前に
            Rotates = Rotates.concat(rot,regRot(NP));
    while (++l<4) ui();
...
// 黄色面を上層にする　
    ...
    $("#proc"+("0"+(n&254)).slice(-2)).attr('disabled',false);
    if ((n==33)&&yon) $("#proc28").attr('disabled',false);
...
// 隣接１組一致なら、隣接交換。対角１組一致なら対角交換。
    if (n<22) {
        $("#proc18").attr('disabled',false);
        ex2corner(22);
        return true;
    }
    if (n<24) {
// 時計読み２連続ムーブ、点ならFRUR'U'F'と時計読み6時ムーブ
        $("#proc20").attr('disabled',false);
