...
(function( $ ) {
    ...
    $.colorPick.prototype = {
        ...
        show: function(element, left, top) {
            ...
            $(element).prepend('<div class="colorPickWrapper"><div id="colorPick" style="display:none;top:' + top + 'px;left:' + left + 'px">');

		if ($.fn.colorPick.defaults.allowRecent === true) {
			$("#colorPick").append('<span style="position: absolute; top: 10px; left:20px; ">Recent:</span>');
			if (JSON.parse(localStorage.getItem("colorPickRecentItems")) == null || JSON.parse(localStorage.getItem("colorPickRecentItems")) == []) {
				$("#colorPick").append('<span class="colorPickButton colorPickDummy"></span>');
			} else {
				jQuery.each(JSON.parse(localStorage.getItem("colorPickRecentItems")), function (index, item) {
				$("#colorPick").append('<span class="colorPickButton" hexValue="' + item + '" style="position: absolute; top: 2px; right: 5px; background:' + item + '"></span>');
                ...
                $("#colorPick").append('<span>'+$.fn.colorPick.defaults.paletteLabel+'</span></div></div>');
...
function capture() {
	if (localMediaStream) {
		...
		for (var y = 6-N; y < 6; y++) {
			for (var x = 6-N; x < 6; x++) {
				var data = ctx.getImageData(10+x*30+15, 10+y*30+15, 10, 10);
...
//    $('#rgb_r').focus();
    if ($('#rgb_w').val()==255) {
         setRgb(180, 110, 110,100, 95);
    setRgbRange(180, 110, 110,100, 95);
    }
