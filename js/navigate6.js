...
let MC3=new Array([0],[1,7,9,3], [2,4,8,6],[3,1,7,9],[4,8,6,2],[5,5,5,5],[6,2,4,8],[7,9,3,1], [8,6,2,4],[9,3,1,7],
                 [10,19,28,37],[11,20,29,38],[12,21,30,39],[13,22,31,40],[14,23,32,41],[15,24,33,42],
                 [16,25,34,43],[17,26,35,44],[18,27,36,45],[19,28,37,10],[20,29,38,11], [21,30,39,12],[22,31,40,13], [23,32,41,14],[24,33,42,15],
                 [25,34,43,16],[26,35,44,17],[27,36,45,18],[28,37,10,19],[29,38,11,20],[30,39,12,21],[31,40,13,22],[32],[33,42,15,24],[34,43,16,25],
                 [35,44,17,26],[36,45,18,27],[37,10,19,28],[38,11,20,29],[39,12,21,30],
                 [40,13,22,31],[41,14,23,32],[42,15,24,33],[43,16,25,34],[44,17,26,35],[45,18,27,36],
                 [46,48,54,52],[47,51,53,49],[48,54,52,46],[49,47,51,53],[50,50,50,50],[51,53,49,47],[52,46,48,54],[53,49,47,51],[54,52,46,48]);
...
function Nc(no,n=N) {  // 仮想3x3Map上で指定パーツの色を求める
    ...
    if (N==2) {
        ...
        else           sftC = layer3N[MC3[no][RotSft]];
        return  Na(sftC);
    } 
}
...
function allTest() {
        ...
        Auto = true;
        scramble();
        if ((N>3)&&(document.opt.noy.checked)) NoRot = " *0c";
        Cool++;
        $("#statusBlk").show();
        if (N<4) NaviIn();
        else next44();
...
function NaviIn(n=0) { // 2x2,3x3型のナビゲーション
    ...
    $("#statusBlk").show();
    RotOmt = 0;
    if ($('input[name="noy"]').is(':checked')) RotOmt = 1;
    if (n==0) turn("");
    SolveNavi(n);
    return true;
}
